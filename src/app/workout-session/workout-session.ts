...
@Component({
  ...
  template: `
    <div class="min-h-screen bg-background text-foreground pb-20">
      <!-- Header -->
      <app-header title="Active Session" [showBackBtn]="true" backLink="/">
      ...
      <main class="container mx-auto px-4 pb-6 max-w-2xl">
        <!-- Active Exercises -->
        <div class="space-y-6">
      ...
      >
        @if (workoutService.hasActiveWorkout()) {
          <button
            ...
            zType="outline"
            zShape="circle"
            class="text-lg flex items-center justify-center gap-2 rounded-full w-full bg-background pointer-events-auto shadow-md"
            (click)="finishWorkout()"
          >
      ...
      @if (isAddSheetOpen()) {
        ...
        >
          <!-- Sheet Header -->
          <div class="p-4 flex justify-between items-center bg-card rounded-t-2xl md:rounded-none border-b border-border">
          ...
          <!-- Sheet Content -->
          <div class="flex-1 overflow-y-auto w-full bg-background">

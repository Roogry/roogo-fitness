...
export const buttonVariants = cva(
  mergeClasses(
    ...
    "text-sm font-medium focus-visible:ring-3 aria-invalid:ring-3 [&_svg:not([class*='size-'])]:size-5 inline-flex items-center",
  ...
  ),
  {
    variants: {
      ...
      zSize: {
        default: 'h-12 gap-2 px-2.5 has-data-[icon=inline-end]:pr-2 has-data-[icon=inline-start]:pl-2',
        ...
        icon: 'size-12',
        'icon-xs': "size-9 rounded-[min(var(--radius-md),10px)] in-data-[slot=button-group]:rounded-lg [&_svg:not([class*='size-'])]:size-4",
        'icon-sm': 'size-10 rounded-[min(var(--radius-md),12px)] in-data-[slot=button-group]:rounded-lg',
        'icon-lg': 'size-14',
      },
      zShape: {
        default: 'rounded-md',
        circle: 'rounded-full px-4',
        square: 'rounded-none',
      },
      zFull: {
        true: 'w-full',
